...
public class Bluetooth {
	...
	public Bluetooth(Handler handler) {
        ...
        if (D) for (BluetoothDevice bd: mAdapter.getBondedDevices()) Log.d(TAG, "Bounded device "+bd + " " + bd.getName());
        mState = STATE_NONE;
        mHandler = handler;
        MY_UUID = UUID.fromString("0001101-0000-1000-8000-00805F9B34FB");
	}

	public Bluetooth(Handler handler, UUID uuid) {
        mAdapter = BluetoothAdapter.getDefaultAdapter();
        if (D) for (BluetoothDevice bd: mAdapter.getBondedDevices()) Log.d(TAG, "Bounded device "+bd + " " + bd.getName());
        mState = STATE_NONE;
        mHandler = handler;
    ...
    public static boolean isBluetoothAvailable() { return BluetoothAdapter.getDefaultAdapter() != null;}
    public static boolean isBluetoothOpen() { return BluetoothAdapter.getDefaultAdapter().isEnabled(); }
    public static void openBluetooth()      { if(!BluetoothAdapter.getDefaultAdapter().isEnabled())  BluetoothAdapter.getDefaultAdapter().enable(); }
    public static void closeBluetooth()     { if(BluetoothAdapter.getDefaultAdapter().isEnabled())  BluetoothAdapter.getDefaultAdapter().disable(); }
	...
	private void connectionLost() {
		...
		mHandler.obtainMessage(MESSAGE_STATE_CHANGE, STATE_DISCONNECT, -1)
				.sendToTarget();
